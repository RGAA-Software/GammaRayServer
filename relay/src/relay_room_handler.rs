...
pub async fn hr_query_room(
    ...
    let mut room_mgr = gRoomMgr.lock().await;
    match room_mgr.as_mut() {
        Some(room_mgr) => {
            let room_id = room_id.unwrap();
            let room = room_mgr.find_room(room_id).await;
            let room = room.unwrap();
            Json(base::ok_resp_str_map(room.as_str_map()))
        }
        None => {
            Json(base::resp_empty_str_map(get_err_pair(ERR_ROOM_NOT_FOUND)))
        }
    }
...
pub async fn hr_query_rooms(
    ...
    match room_mgr.as_mut() {
        Some(room_mgr) => {
            let room_ids = room_mgr.find_room_ids(page, page_size).await;
            let mut r = Vec::new();
        ...
        None => {
            Json(base::resp_empty_vec_str_map(get_err_pair(ERR_ROOM_NOT_FOUND)))
        }
    }
